"""ADS-B blueprint module for the WaveTap API."""
...
def _get_db_path() -> Path:
    try:
        configured = current_app.config.get("ADSB_DB_PATH")  # type: ignore[attr-defined]
    except RuntimeError:
        configured = None
    if configured:
...
def _ensure_schema(conn: sqlite3.Connection, db_path: Path) -> None:
    if _SCHEMA_INITIALIZED.get(db_path):
        return
...
@adsb_bp.app_template_filter("format_coord")
...
@adsb_bp.route("/live", methods=["GET"])
def live_data():
...
@adsb_bp.route("/flight-paths", methods=["GET"])
def flight_paths():
...
@adsb_bp.route("/live-map", methods=["GET"])
def live_map():
    aircraft = _filter_recent_aircraft()
